	...
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") ИЛИ (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				Если (СтрСтроки.Статус = "Failed") ИЛИ (СтрСтроки.Статус = "Pending") ИЛИ ТекСтатус = "Failed" ИЛИ ТекСтатус = "Pending" Тогда
					ОписаниеОшибки = "";
					Если РезультатПрохожденияТестовСценария.Свойство("ОписаниеОшибки") Тогда
						ОписаниеОшибкиСценария = РезультатПрохожденияТестовСценария.ОписаниеОшибки;
					Иначе
...
Функция ВывестиОтчетТестированияВоВнутреннемФормате(Дерево, ПараметрыОтчетаУФ) Экспорт
	...
	ВА_ВанессаСервер.ДействияПослеВыполненияТестов(Дерево, ПараметрыОтчетаУФ, ЭтотОбъект); //#DIP-29 #ITC КурцовАИ 16.08.2022 
