...
Процедура ОчиститьЗаписиРегистра(ПараметрыОтбора)
	...
	НаборЗаписейКонфликта = НаборЗаписейРегистраПоПараметрамОтбора(ПараметрыОтбора, Метаданные.РегистрыСведений._РезультатыОбменаДанными);
	НаборЗаписейКонфликта.Записать();
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	//++Волкова ругается при обновлении,регистр из которого переносит данные все равно пустой, поэтому просто возврат
	Возврат;
	//--
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		Возврат;
